                ...
                if (is_array($primaryKey)) {
                    ...
                    foreach ($primaryKey as $pKey) {
                        if (!isset($formData[$pKey])) {
                            throw new PluginException('主键（' . $pKey . '）缺失！');
                        }
                    ...
                    if (!isset($formData[$primaryKey])) {
                        throw new PluginException('主键（' . $primaryKey . '）缺失！');
                    }
        ...
        if (isset($postData['selectedRows'])) {
            ...
            try {
                ...
                if (!is_array($postData['selectedRows']) || count($postData['selectedRows']) == 0) {
                    throw new PluginException('你尚未选择要操作的数据！');
                }

                if (!isset($postData['postData']['value'])) {
                    throw new PluginException('参数（postData.value）缺失！');
                }
                ...
                foreach ($postData['selectedRows'] as $row) {
                    ...
                    if (is_array($primaryKey)) {
                        ...
                        foreach ($primaryKey as $pKey) {
                            if (!isset($row[$pKey])) {
                                throw new PluginException('主键（selectedRows[' . $i . '].' . $pKey . '）缺失！');
                            }
                        ...
                        if (!isset($row[$primaryKey])) {
                            throw new PluginException('主键（selectedRows[' . $i . '].' . $primaryKey . '）缺失！');
                        }
            ...
            try {
                ...
                if (!isset($postData['row'][$field])) {
                    throw new PluginException('参数（row.' . $field . '）缺失！');
                }
