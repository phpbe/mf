<?php
namespace Be\Mf\Cache\Template\Admin\Plugin\Lists;

use Be\Mf\Be;

class display extends \Be\F\Template\Driver
{
...
<body>
    ...
    <div id="app" v-cloak>
        ...
        ?>>
            ...
            if (isset($this->setting['tab'])) {
                $driver = new \Be\Mf\Plugin\Tab\Driver($this->setting['tab']);
                ...
                if ($vueDataX) {
                    $vueData = \Be\F\Util\Arr::merge($vueData, $vueDataX);
                }
            ...
            if (isset($this->setting['toolbar']['items']) && count($this->setting['toolbar']['items']) > 0) {
                ...
                foreach ($this->setting['toolbar']['items'] as $item) {
                    ...
                    if (isset($item['driver'])) {
                        ...
                        $driver = new \Be\Mf\Plugin\Toolbar\Item\ToolbarItemButton($item);
                    }
            ...
            ?>>
                ...
                if (isset($this->setting['operation'])) {
                    ...
                    $operationDriver = new \Be\Mf\Plugin\Operation\Wrap($this->setting['operation']);
                    ...
                    if (isset($this->setting['operation']['items'])) {
                        foreach ($this->setting['operation']['items'] as $item) {
                            ...
                            if (isset($item['driver'])) {
                                ...
                                $driver = new \Be\Mf\Plugin\Operation\Item\OperationItemLink($item);
                            }
    ...
    if (isset($this->setting['vueData'])) {
        $vueData = \Be\F\Util\Arr::merge($vueData, $this->setting['vueData']);
    }

    if (isset($this->setting['vueMethods'])) {
        $vueMethods = \Be\F\Util\Arr::merge($vueMethods, $this->setting['vueMethods']);
    }
