...
use Be\Mf\Be;
use Be\Mf\Plugin\Form\Item\FormItemFile;
...
class Importer extends Driver
{
    ...
    public function setting($setting = [])
    {
        ...
        if (!isset($setting['mapping']['items'])) {
            throw new PluginException('导入表格（mapping.items）映射项缺失！');
        }

        foreach ($setting['mapping']['items'] as $index => $item) {
            if (!isset($item['name'])) {
                throw new PluginException('第 ' . ($index + 1) . ' 项字段配置错误：未设置列名称（name）！');
            }

            if (!isset($item['label'])) {
                throw new PluginException('第 ' . ($index + 1) . ' 项字段配置错误：未设置列表头（label）！');
            }
        }
    ...
    public function process()
    {
        ...
        if ($type == 'csv') {
            ...
            if ($headerCount == 0) {
                throw new PluginException('您上传的文件中无数据！');
            }
            ...
            if (count($errors) > 0) {
                throw new PluginException('有' . count($errors) . '条数据有问题！');
            }
            ...
            if (!$reader || !$reader->canRead($path)) {
                throw new PluginException('上传的文件不是有效的Excel文件！');
            }
            ...
            throw new PluginException('不支持的文件类型：' . $type . '！');
        }
    }
